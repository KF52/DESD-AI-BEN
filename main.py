...
if os.path.exists(active_model_path):
    try:
        models["active"] = joblib.load(active_model_path)
...
async def reload_models(current_user: dict = Depends(get_current_user)):
    """Reload all models without restarting the API"""
    try:
        ...
        if os.path.exists(active_model_path):
            try:
                models["active"] = joblib.load(active_model_path)
                print("Active model reloaded successfully")
            except Exception as e:
                print(f"Error reloading active model: {str(e)}")
                return {"success": False, "error": f"Error reloading active model: {str(e)}"}        
        return {
            "success": True, 
